<footer class="ftco-footer ftco-bg-dark ftco-section">
      <div class="container">
        <div class="row mb-5">
          <div class="col-md">
            <div class="ftco-footer-widget mb-5">
              <h2 class="ftco-heading-2 logo">Triage<span>System</span></h2>
              
            </div>
            
          </div>
          <div class="col-md">
            <div class="ftco-footer-widget mb-5 ml-md-4">
              <h2 class="ftco-heading-2 logo">Login  </h2>
              <ul class="list-unstyled">
              
              <li><a href="adminlogin.php"><span class="ion-ios-arrow-round-forward mr-2"></span>Admin Login </a></li>
                <li><a href="doctorlogin.php"><span class="ion-ios-arrow-round-forward mr-2"></span>Doctor Login </a></li>
                <li><a href="chwlogin.php"><span class="ion-ios-arrow-round-forward mr-2"></span>Community Healthcare Worker Login </a></li>

              </ul>
            </div>
          
          </div>
          
             
      </div>
    </footer>