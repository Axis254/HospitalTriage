...
if(isset($_POST[submit]))
{
	...
	{
		$sql ="INSERT INTO appointment(patientid,sugarlevel,bloodpressure,appointmentdate,appointmenttime,app_reason,status,departmentid,doctorid) values('$lastinsid','$_POST[sugarlevel]','$_POST[bloodpressure]','$_POST[appointmentdate]','$_POST[appointmenttime]','$_POST[app_reason]','Pending','$_POST[department]','$_POST[doct]')";
		if($qsql = mysqli_query($con,$sql))
		{
...
<html lang="en">
  <head>
  <title>Patient Appointment | Triage System</title>
<meta charset="UTF-8">
...
else
{
 ?>
   <form method="post" action="" name="frmpatapp" onSubmit="return validateform()">
    <table align="center" width="532" border="3">
      <tbody>
        <tr>
...
?>
		...
		<tr>
         <strong> <td>Sugar Level</td> </strong>
          <td><input type="text" name="sugarlevel" id="sugarlevel"> </td>
		  <tr>
         <strong> <td>Blood Pressure</td> </strong>
          <td><input type="text" name="bloodpressure" id="bloodpressure"> </td>
        </tr>        
        </tr> 
        <tr>
          <td>DOB</td>
...
function validateform()
{
	...
	else if(document.frmpatapp.sugarlevel.value == "")
	{
		alert("Sugar level should not be empty..");
		document.frmpatapp.appointmenttime.focus();
		return false;
	}
	else if(document.frmpatapp.bloodpressure.value == "")
	{
		alert("Blood pressure should not be empty..");
		document.frmpatapp.bloodpressure.focus();
		return false;
	}
	else
	{
